...
export function listValidMoves(index: number, piece: string | null, board: BoardType, team: string) {
  ...
  const pos: PositionType = [Math.floor(index / 8), index % 8];
  if (piece === null) {
    // error message 
...
function pawn_moves(pos: PositionType, team: string, board: BoardType) {
  ...
  if (team == 'white') {
    if (pos[0] - 1 >= 0) {
      ...
      if (pos[1] - 1 >= 0) {
        const piece = board[pos[0] - 1][pos[1] - 1].piece;
...
function knight_moves(pos: PositionType, board: BoardType, team: string): number[] {
  ...
  const filtered: PositionType[] = all_moves.filter((ele: PositionType): boolean => !(ele[0] < 0 || ele[0] > 7 || ele[1] < 0 || ele[1] > 7));
  const blocked: PositionType[] = filtered
    .filter((ele: PositionType) =>
      board[ele[0]][ele[1]].piece !== null && same_team(board[ele[0]][ele[1]].piece, team)
    );
  const finalMoves: PositionType[] = filtered.filter(ele => !blocked.includes(ele));
...
function bishop_moves(pos: PositionType, board: BoardType, team: string): number[] {
  const diagonal: PositionType[] = diagonal_squares(pos);
  const occupied: PositionType[] = diagonal.filter(ele => board[ele[0]][ele[1]].piece !== null);
  let blocked: PositionType[] = [];
  for (let i = 0; i < occupied.length; i++) {
    const square = occupied[i];
    const x = square[0];
    const y = square[1];
    if (x < pos[0] && y > pos[1]) {
...
function rook_moves(pos: PositionType, board: BoardType, team: string) {
  ...
  const orthogonal_squares_without_ownpos = column_moves.concat(row_moves);
  // find blocking squares
  const occupied_squares: PositionType[] = orthogonal_squares_without_ownpos
    .filter(ele => board[ele[0]][ele[1]].piece !== null);
  for (const occupied of occupied_squares) {
    if (occupied[1] === pos[1]) {
      //same column 
      if (occupied[0] < pos[0]) {
...
export function move(from: number, to: number, board: BoardType, legalMoves: number[]): [BoardType, string | null] {
  const movingPiece = board[Math.floor(from / 8)][from % 8].piece;
  const dstPiece = board[Math.floor(to / 8)][to % 8].piece;
